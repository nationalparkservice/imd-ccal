    ...
    x[[1]] %>% # process CCAL data with function from original package
      ...
      dplyr::mutate(Activity_ID = NA,
             ...
             Result_Comment = dplyr::case_when(value %<<% Method_Detection_Limit
                                               ...
                                               TRUE ~ ""),
             Lab_Comment = {if("comment" %in% names(.)) comment else NA},
             Lab_Comment = dplyr::if_else(is.na(Lab_Comment), "", paste0("Note from lab: ", Lab_Comment, ";")),
             Result_Comment = stringr::str_trim(paste0(Result_Comment, Lab_Comment)),
             Result_Comment = dplyr::if_else(Result_Comment == "", NA, Result_Comment),
